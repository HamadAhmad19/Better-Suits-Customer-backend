...
const TaxiLaterScreen = () => {
  const navigation = useNavigation();
  const route = useRoute();
  const { user } = useAuth();
  const phoneNumber = user?.phoneNumber;

  // Get default pickup from route params
  const { defaultPickup } = route.params || {};
  ...
  // Set default pickup when component mounts or when defaultPickup changes
  useEffect(() => {
    if (defaultPickup) {
      setSelectedPickup(defaultPickup);

      // Use the actual address or fallback to "Current Location"
      const pickupText = defaultPickup.address ||
        defaultPickup.name ||
        "Current Location";
      setPickup(pickupText);

      // Animate map to default pickup location
      if (mapRef.current) {
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener('focus', () => {
      console.log('TaxiLaterScreen focus - route params:', route.params);
      ...
      if (route.params?.selectedPickup) {
        ...
        setPickup(route.params.pickupName || route.params.selectedPickup.address || '');
      }
      if (route.params?.selectedDropoff) {
        setSelectedDropoff(route.params.selectedDropoff);
        setDropoff(route.params.dropoffName || route.params.selectedDropoff.address || '');
      }
    });
    ...
    try {
      ...
      if (json.status === 'OK' && json.result) {
        ...
        if (field === 'pickup') {
          ...
          navigation.navigate('PickupLater', {
          ...
          // navigate to Dropoff screen - pass the full pickup object, not just string
          navigation.navigate('DropoffLater', {
            selectedLocation: location,
            locationName: formatted_address,
            phoneNumber: phoneNumber,
            selectedPickup: selectedPickup, // Pass the full pickup object
            pickupName: pickup, // Pass the pickup name as well
          });
        }
      } else {
    ...
    <View style={{ flex: 1, backgroundColor: '#fff' }}>
      ...
      >
        <View style={styles.header}>
          <TouchableOpacity onPress={() => navigation.navigate('MapPage', { phoneNumber })}>
            <Entypo name="cross" size={22} color="black" />
          </TouchableOpacity>
          <Text style={styles.title}>Pre-Booking</Text>
        </View>
        ...
        <View style={styles.inputContainer}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              // Navigate to Pickup screen with current selection
              navigation.navigate('PickupLater', {
        ...
        <View style={styles.inputContainer}>
          <View style={styles.inputIcon}>
            <Feather name="map-pin" size={16} color="#FF3B30" />
          </View>
          <TextInput
            style={styles.input}
            placeholder="Enter drop-off point"
            value={dropoff}
            onChangeText={(t) => onChangeTextDebounced(t, 'dropoff')}
            onFocus={() => {
              setActiveField('dropoff');
              if (dropoff.length > 2) fetchAutocomplete(dropoff);
            }}
          />
          <TouchableOpacity
            style={styles.mapButton}
            onPress={() => {
              // Navigate to Dropoff screen with current selection
              navigation.navigate('DropoffLater', {
                selectedLocation: selectedDropoff,
                locationName: dropoff,
                phoneNumber: phoneNumber,
                selectedPickup: selectedPickup, // Pass full pickup object
                pickupName: pickup, // Pass pickup name
              });
            }}
          >
            <Text style={styles.mapButtonText}>Map</Text>
        ...
        <View style={styles.confirmWrapper}>
          <TouchableOpacity
            style={[
              ...
              (!selectedPickup || !selectedDropoff) && styles.confirmButtonDisabled,
            ]}
            onPress={() => {
...
export default TaxiLaterScreen;
