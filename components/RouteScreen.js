...
//         // Fit map after a short delay to ensure coordinates are set
//         setTimeout(() => {
//           fitMapToMarkers();
//         }, 300);
//       }
...
//           x: point.x,
//           y: point.y - 60, // Position above the marker
//         });
//       }).catch(error => {
...
//   const handleMarkerPress = (type, coordinate) => {
//     if (mapRef.current) {
...
//     const ridePanelTop = height * 0.4; // Ride panel starts at 40% from top
//     const fadeStart = ridePanelTop - 120; // Start fading 120px above panel for smoother transition

//     if (yPosition < fadeStart) return 1;
//     if (yPosition > ridePanelTop) return 0;

//     // Linear fade between fadeStart and ridePanelTop
//     return 1 - ((yPosition - fadeStart) / (ridePanelTop - fadeStart));
//   };

//   // Helper function to keep label within screen bounds
//   const constrainLabelPosition = (xPosition, labelWidth = 150) => {
//     const margin = 20; // Safe margin from screen edges
//     const halfWidth = labelWidth / 2;

//     // Calculate desired left position (centered on marker)
//     let leftPosition = xPosition - halfWidth;

//     // Constrain to screen bounds with margin
//     if (leftPosition < margin) {
//       leftPosition = margin;
//     } else if (leftPosition + labelWidth > width - margin) {
//       leftPosition = width - margin - labelWidth;
//     }
...
//         <TouchableOpacity onPress={() => navigation.navigate('Taxi', { phoneNumber })} style={styles.backButton}>
...
//           <Marker
//             coordinate={pickup}
//             onPress={() => { navigation.navigate('Taxi', { pickup }) }}
//           >
...
//             coordinate={dropoff}
//             onPress={() => {
//               navigation.navigate('Taxi', { dropoff })
//             }}
//           >
//             <View style={{ width: 30, height: 30 }}>
//               <Image
...
//             {
//               left: constrainLabelPosition(pickupScreenPosition.x, 150),
//               top: pickupScreenPosition.y,
//               opacity: calculateLabelOpacity(pickupScreenPosition.y),
...
//             {
//               left: constrainLabelPosition(dropoffScreenPosition.x, 150),
//               top: dropoffScreenPosition.y,
//               opacity: calculateLabelOpacity(dropoffScreenPosition.y),
...
//     position: 'absolute',
//     bottom: height * 0.68, // Position it above the service container
//     right: 20,
//     width: 40,
//     height: 40,
//     borderRadius: 25,
...
const RouteScreen = () => {
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              // Instead of creating booking here, we pass all details to ConfirmRideScreen
              navigation.navigate("ConfirmRide", {
                pickup: pickup,
                dropoff: dropoff,
                selectedRide: selectedRide,
                ...
                estimatedTime: estimatedTime,
                estimatedDistance: estimatedDistance,
                // Pass preferences to next screen
                preferences: {
                  waitTime: waitTime,
                  luggage: luggage,
                  pet: pet,
  ...
  const calculateLabelOpacity = (yPosition) => {
    const ridePanelTop = height * 0.4; // Ride panel starts at 40% from top
    const fadeStart = ridePanelTop - 100; // Start fading 100px above panel
      ...
      >
        {pickup && (
          <Marker
            coordinate={pickup}
            onPress={() => {
              navigation.navigate("Taxi", { pickup });
            }}
          >
            <View style={{ width: 30, height: 30 }}>
              <Image
        ...
        {routeCoordinates.length > 0 && (
          <Marker coordinate={getRouteMidPoint()} anchor={{ x: 0.5, y: 1.5 }}>
            <View style={styles.timeLabel}>
              <Text style={styles.timeLabelText}>{estimatedTime}</Text>
            </View>
          </Marker>
        )}
      </MapView>
      ...
      {pickupScreenPosition && (
        <View
          style={[
            ...
            styles.pickupLabelContainer,
            {
              left: pickupScreenPosition.x - 75, // Center the label
              top: pickupScreenPosition.y,
      ...
      {dropoffScreenPosition && (
        <View
          style={[
            ...
            styles.dropoffLabelContainer,
            {
              left: dropoffScreenPosition.x - 75, // Center the label
              top: dropoffScreenPosition.y,
            ...
            <View style={{ paddingHorizontal: 20 }}>
              ...
              <View>
                <TouchableOpacity
                  style={styles.prefRow}
                  onPress={() =>
                    setWaitTimeDropdownVisible(!waitTimeDropdownVisible)
                  }
                >
                  <Text style={styles.prefLabel}>Wait time</Text>
                  <Text style={styles.prefSelect}>
                    {waitTime ? waitTime : "Select"}
                  </Text>
                </TouchableOpacity>
