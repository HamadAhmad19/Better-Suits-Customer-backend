...
const SaveWorkScreen = () => {
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener('focus', () => {
      ...
      // Check for defaultPickup passed from MapScreen
      if (route.params?.defaultPickup) {
        setSelectedPickup(route.params.defaultPickup);
        setPickup(route.params.defaultPickup.address || route.params.defaultPickup.name || '');
        // Clear the param so it doesn't overwrite if we navigate back and forth without intent
        navigation.setParams({ defaultPickup: undefined });
      }
    });
  ...
  const handleMapButton = () => {
    navigation.navigate('WorkLocation', { phoneNumber });
  };
    ...
    <View style={{ flex: 1, backgroundColor: '#fff' }}>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Save Your Work Location</Text>
        </View>
        {/* Pickup input */}
        <View style={styles.inputContainer}>
          ...
          />
          {/* <TouchableOpacity style={styles.mapButton} onPress={handleMapButton}>
            ...
            onPress={() => {
              if (selectedPickup) {
                navigation.navigate('WorkLocation', {
                  ...
                  locationName: pickup,
                  phoneNumber, // Ensure phone number is passed if needed
                });
              }
            }}
