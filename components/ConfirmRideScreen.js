...
import { createBooking, cancelBooking, updateBookingPreferences } from "../services/apiService";
...
const ConfirmRideScreen = () => {
  ...
  const [isRidePreferencesModalVisible, setIsRidePreferencesModalVisible] =
    useState(false);
  ...
  const [isCancelRideModalVisible, setIsCancelRideModalVisible] =
    useState(false);
  ...
  // Ride Preferences States
  const [waitTime, setWaitTime] = useState(preferences?.waitTime || "Select");
  const [waitTimeDropdown, setWaitTimeDropdown] = useState(false);

  // Flag to prevent duplicate booking creation
  ...
  const [
    ...
    setIsPaymentConfirmationModalVisible,
  ] = useState(false);
  // For auto-scrolling
  ...
  const [messageCategories, setMessageCategories] = useState([
    { id: 'location', title: 'Location', icon: 'location-pin' },
    { id: 'arrival', title: 'Arrival', icon: 'timer' },
    { id: 'requests', title: 'Requests', icon: 'help' },
    { id: 'payment', title: 'Payment', icon: 'payment' },
  ]);
  const [activeMessageCategory, setActiveMessageCategory] = useState('location');
  ...
  const findAvailableDrivers = () => {
    ...
    const filteredDrivers = DUMMY_DRIVERS.filter(
      (driver) =>
        driver.rideType === selectedRide &&
        driver.isOnline &&
  ...
  const playSuccessSound = async () => {
    try {
      ...
      const { sound: newSound } = await Audio.Sound.createAsync(
        require('../assets/sounds/success-beep.mp3'), // Add your sound file
        { shouldPlay: true }
      );

      setSound(newSound);

      // Set volume and playback options
      await newSound.setVolumeAsync(0.7); // Adjust volume (0.0 to 1.0)
  ...
  // Add this useEffect to play sound when ride status changes to driver_found
  useEffect(() => {
  ...
  const sendDriverRequest = (driver) => {
    ...
    const timer = setTimeout(async () => {
      ...
      // CREATE BOOKING HERE (Saved to DB only when driver is assigned)
      // Guard: Only create booking if not already created
      if (!bookingCreated) {
        try {
          setBookingCreated(true); // Set flag immediately to prevent duplicates
          ...
          const bookingData = {
            ...
            preferences: {
              ...
              waitTime: waitTime !== "Select" ? waitTime : null,
            },
          };

          console.log("✅ Driver Found! Creating booking...", bookingData);
          const response = await createBooking(bookingData);

          if (response && response.bookingId) {
            setCurrentBookingId(response.bookingId);
            console.log("🆔 Booking ID stored:", response.bookingId);
          }
        } catch (error) {
          console.error("Failed to save booking:", error);
          setBookingCreated(false); // Reset flag on error so user can retry
          // Optional: Alert user, but keep flow running for demo
        }
      } else {
        console.log("⚠️ Booking already created, skipping duplicate creation");
      }
  ...
  // Function to update preferences in the database
  const handleUpdatePreferences = async () => {
    if (!currentBookingId) {
      console.log("⚠️ No booking ID, skipping preference update");
      return;
    }
    ...
    try {
      ...
      await updateBookingPreferences(currentBookingId, updatedPreferences);
      console.log("✅ Preferences updated successfully");
    } catch (error) {
      console.error("❌ Failed to update preferences:", error);
      // Silently fail - user can continue ride
    }
  };
  ...
  const shareTrip = async () => {
    try {
      await Share.share({
        message: `I am on my way to ${actualDropoff.name}`,
      });
    } catch (error) {
      console.log(error);
    }
  ...
  const sendMessage = () => {
    ...
    setMessages((prev) => [...prev, { text: typedMessage, sender: "me" }]);
  ...
  const sendPredefinedMessage = (message) => {
    ...
    setTimeout(() => {
      const driverResponses = [
        "Got it, thanks!",
        "I see you, I'll be there shortly",
        "On my way, 2 minutes away",
        "I'm at the pickup spot, where are you?",
        "I'll wait for you",
        "No problem, I'll wait",
        "Sure, I can help with that",
        "Thanks for letting me know",
        "I'll turn on the AC/heat",
        "I'll come to the main entrance",
      ];
      const randomReply = driverResponses[Math.floor(Math.random() * driverResponses.length)];
    ...
    // Clear input if you're using it
    setTypedMessage("");
  };
      ...
      <View style={styles.stepContainer1}>
        ...
        <View style={styles.locationCard1}>
          <View style={styles.locationRow1}>
            <Text style={styles.locationDropText1}>
              Dropoff: {step.content.dropoff}
            </Text>
          </View>
        ...
        {availableDrivers.length > 0 ? (
          ...
          >
            ...
            {availableDrivers.map((driver, index) => (
              ...
              >
                <Image source={driver.image} style={styles.verySmallImage} />
                <View style={{ flexDirection: "column", marginLeft: 15 }}>
                  <Text style={styles.driverName}>{driver.name}</Text>
                  <Text style={styles.driverRating}>⭐ {driver.rating}</Text>
                  <Text style={styles.driverLocation}>
                    {driver.location} • {driver.distance} km
                  </Text>
        ...
        return (
          <View style={styles.pickupCard}>
            ...
            <View style={styles.pickupDriverSection}>
              ...
              <View style={styles.pickupDriverInfo}>
                <View style={styles.driverTextContainer}>
                  <Text style={styles.pickupDriverNameLarge}>
                    {selectedDriver?.name}
                  </Text>
                  <Text style={styles.pickupDriverRatingBadge}>
                    ⭐{selectedDriver?.rating} ({selectedDriver?.rideType})
                  </Text>
                </View>
            ...
            <View style={styles.vehicleContainer}>
              <View>
                <Text style={styles.pickupVehiclePlate}>
              ...
              <View style={styles.ImgContainer}>
                <Image
                  source={require("../assets/BookedCar.png")}
                  style={styles.Img}
                />
              </View>
            </View>
            ...
            <View style={styles.pickupOptionsRow}>
              ...
              >
                <Ionicons
                  ...
                  style={{ marginRight: 5 }}
                />
                <Text style={styles.pickupOptionItem}>Ride options</Text>
              </TouchableOpacity>
              <View style={styles.pickupOptionSeparator} />
              <TouchableOpacity
                style={{ flexDirection: "row" }}
                onPress={() => setIsRideSafetyVisible(true)}
              >
                <MaterialIcons
                  name="safety-check"
                  size={20}
                  ...
                  style={{ marginRight: 5 }}
                />
                <Text style={styles.pickupOptionItem}>Ride Safety</Text>
              </TouchableOpacity>
            </View>
          </View>
        );
  ...
  const RidePreferencesModal = () => {
    // Use parent's waitTime state instead of local state
    // Removed: const [waitTimeDropdown, setWaitTimeDropdown] = useState(false);
    // Removed: const [selectedWaitTime, setSelectedWaitTime] = useState(preferences?.waitTime || "Select");
            ...
            >
              ...
              <View style={styles.sectionContainer}>
                <View style={styles.row}>
                  ...
                  >
                    <MaterialIcons
                      ...
                      style={{
                        ...
                        borderWidth: 1,
                        borderColor: "rgba(186, 184, 186, 0.5)",
                        borderRadius: 8,
                        marginRight: 5,
                      }}
                    />
                  ...
                  >
                    <Text style={styles.dropdownText}>{waitTime}</Text>
                    ...
                    {waitTimeOptions.map((item, index) => (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          ...
                          setTimeout(() => handleUpdatePreferences(), 100);
                        }}
                      >
              ...
              <TouchableOpacity
                ...
                onPress={() => setIsModalCouponVisible(true)}
              >
              ...
              <TouchableOpacity
                style={styles.menuItem}
                onPress={() => setIsCancelRideModalVisible(true)}
              >
                <MaterialIcons
                  name="cancel"
                  size={20}
  ...
  return (
    <View style={styles.container}>
        ...
        {selectedDriver && rideStatus !== "searching" && (
          ...
          >
            <View style={styles.driverMarkerContainer}>
              <Image
                source={require("../assets/CarPointer3.png")}
      ...
      {rideStatus !== "searching" ? (
        <View style={styles.rideStatusOverlay1}>{renderRideStatus()}</View>
      ) : (
      ...
      >
        <View style={styles.modalOverlayRedeem}>
          <View style={styles.modalContainerRedeem}>
            ...
            {/* Redeem Button */}
            <TouchableOpacity style={styles.redeemButtonRedeem} onPress={() => {
              setIsModalCouponVisible(false);
            ...
            <TouchableOpacity
              ...
              onPress={async () => {
                try {
                  if (currentBookingId) {
                    console.log(
                      "🛑 Cancelling ride with ID:",
                      currentBookingId
                    );
                    await cancelBooking(currentBookingId);
                    console.log("✅ Ride cancelled and deleted from DB.");
                  } else {
                    console.warn(
                      "⚠️ No booking ID found to delete. Just navigating back."
                    );
                  }
                } catch (error) {
        ...
        <View style={styles.rideSafetyOverlay}>
          <View style={styles.rideSafetyContainer}>
            ...
            >
              ...
              <View>
                <Text style={styles.rideSafetyOptionTitle}>
        ...
        <View style={styles.sosEmergencyOverlay}>
          <View style={styles.sosEmergencyContainer}>
            ...
            <Text style={styles.sosWarningText}>
              IMPORTANT: Please use this feature only in case of emergency. We
              will contact authorities on your behalf.
            </Text>
      ...
      >
        <View style={styles.reportIssueOverlay}>
          <View style={styles.reportIssueContainer}>
            ...
            <TouchableOpacity
              style={styles.reportIssueSubmitBtn}
              onPress={() => setIsReportIssueVisible(false)}
            >
              <Text style={styles.reportIssueSubmitText}>
                Report this issue
              </Text>
            </TouchableOpacity>
          </View>
      ...
      >
        <View style={styles.chatModalContainer}>
          ...
          <ScrollView
            ...
            contentContainerStyle={{ paddingBottom: 150 }} // Increased for extra UI
            onContentSizeChange={() =>
              scrollViewRef.current?.scrollToEnd({ animated: true })
            }
          >
              ...
              >
                {messageCategories.map((category) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.categoryTab,
                      activeMessageCategory === category.id && styles.categoryTabActive,
                    ]}
                    onPress={() => setActiveMessageCategory(category.id)}
                  >
                    {category.icon && (
                      <MaterialIcons
                        name={category.icon}
                        size={16}
                        color={activeMessageCategory === category.id ? "#0254E8" : "#666"}
                        style={{ marginRight: 5 }}
                      />
                    )}
                    <Text
                      style={[
                        ...
                        activeMessageCategory === category.id && styles.categoryTabTextActive,
                      ]}
                    >
                      {category.title}
                    </Text>
          ...
          {/* Predefined Messages Row */}
          <View style={styles.predefinedMessagesContainer}>
            <ScrollView
              horizontal
              showsHorizontalScrollIndicator={false}
              style={styles.predefinedMessagesScroll}
              contentContainerStyle={styles.predefinedMessagesContent}
            >
              {predefinedMessages[activeMessageCategory]?.map((msg, index) => (
                <TouchableOpacity
                  key={index}
            ...
            <View style={styles.reviewToggleContainer}>
              <TouchableOpacity
                style={[
                  ...
                  styles.reviewToggleButtonActive,
                ]}
                  ...
                  {positiveWords.map((word) => (
                    <TouchableOpacity
                      ...
                      style={[
                        ...
                        styles.reviewWordButtonSelected,
                      ]}
          ...
          <View style={styles.modalContainer}>
            ...
            <View style={{ padding: 20 }}>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  setIsModalGiftCardVisible(false);
                  handleUpdatePreferences();
                }}
              >
...
const styles = StyleSheet.create({
  ...
  predefinedMessagesScroll: {
    maxHeight: 70,
  },

  predefinedMessagesContent: {
    alignItems: "center",
    paddingVertical: 8,
  },
  ...
  // Update chatInputRow for more space
  chatInputRow: {
    flexDirection: "row",
    alignItems: "center",
    padding: 12,
  ...
  // Update chatModalMessages for more content space
  chatModalMessages: {
    flex: 1,
    marginTop: 10,
    paddingHorizontal: 10,
