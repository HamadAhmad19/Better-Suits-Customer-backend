...
const PickupLaterScreen = () => {
  const navigation = useNavigation();
  const route = useRoute();
  const { selectedLocation: initialLocation, locationName: initialLocationName } = route.params || {};
  const { user } = useAuth();
  ...
  const centerOnUserLocation = async () => {
    try {
      ...
      // Optionally reverse geocode the user's current location to set it as selected
  ...
  const handleConfirm = () => {
    if (selectedLocation) {
      navigation.navigate({
        name: 'TaxiLater',
        params: {
          selectedPickup: selectedLocation,
          pickupName: searchQuery,
          phoneNumber: route.params?.phoneNumber,
          // Preserve existing dropoff if any
          selectedDropoff: route.params?.selectedDropoff,
          dropoffName: route.params?.dropoffName
        },
        merge: true // This is key - it merges with existing params
      });
    }
  };
      ...
      <View style={styles.searchSection}>
        ...
        <View style={styles.searchContainer}>
          <View style={styles.searchInputContainer}>
            ...
            />
            {isLoadingAddress && (
              <ActivityIndicator size="small" color="#0254E8" />
            )}
          </View>
        ...
        >
          <Text style={styles.confirmButtonText}>
            {isLoadingAddress ? 'Getting Location...' : 'Confirm pick-up Location'}
          </Text>
        </TouchableOpacity>
      </View>
    </View>
...
export default PickupLaterScreen;